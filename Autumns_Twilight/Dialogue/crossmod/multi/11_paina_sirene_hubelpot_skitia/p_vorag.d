...
== P_ChonJ IF ~InParty("P_Chon") InMyArea("P_Chon") !StateCheck("P_Chon",CD_STATE_NOTVALID)~ THEN @11100179 /* Tilabil Voragor, te he visto en combate cuando estábamos en el Abismo. Eres más que capaz de ejecutar lo que os digo. */
...
// Paina Crossmod
== C0PAINAJ IF ~InParty("C0Paina") InMyArea("C0Paina") !StateCheck("C0Paina",CD_STATE_NOTVALID)~ THEN @10000000 /* ~Hmpf. ¿Por qué lo piensas tanto, <CHARNAME>? La criatura no nos atacó. Además de que es un ser inteligente y pensante. Lo digo más por sentido común que por otra cosa.~ */


// Sirene Crossmod
== C0SIRE2J IF ~InParty("C0Sirene") InMyArea("C0Sirene") !StateCheck("C0Sirene",CD_STATE_NOTVALID)~ THEN @10000048 /* ~No hemos oído ataques de un basilisco en el valle, <CHARNAME>. Además, es una criatura capaz de comunicarse y claramente inteligente. Matarle sería una injusticia.~ */

// Hubelpot crossmod
== SC#HubJ IF ~InParty("Hubelpot") InMyArea("Hubelpot") !StateCheck("Hubelpot",CD_STATE_NOTVALID)~ THEN @10000192 /*~Todo lo que puedo decir es que nunca he cocinado a un basilisco, <CHARNAME>. Puede que algo bueno se pueda hacer con su carne y con sus escamas quizás algo más acorde a las necesidades tuyas o algo así. Aunque si quieres evitar la pelea, por Chauntea que no escucharás quejas de mi parte.~ */

// SKITIA NPCS
== X3EMIJ IF ~InParty("X3Emi") InMyArea("X3Emi") !StateCheck("X3Emi",CD_STATE_NOTVALID)~ THEN @11100567 /* ~No parece una criatura sedienta de sangre, <CHARNAME>. Creo que deberíamos dejarle con vida.~ */
== X3HELJ IF ~InParty("X3Hel") InMyArea("X3Hel") !StateCheck("X3Hel",CD_STATE_NOTVALID)~ THEN @11100568 /* ~Nunca he visto a un basilisco anciano no atacar a sus presas. Sin embargo, admito que sería glorioso entrar en combate con uno. Piénsalo, <CHARNAME>.~ */
== X3KALJ IF ~InParty("X3Kal") InMyArea("X3Kal") !StateCheck("X3Kal",CD_STATE_NOTVALID)~ THEN @11100569 /* ~Entonces... ¿todos ignorarán el hecho de que este basilisco habla? Si podemos evitar un combate innecesario, es algo inteligente a hacer. ~ */
== X3REBJ IF ~InParty("X3Reb") InMyArea("X3Reb") !StateCheck("X3Reb",CD_STATE_NOTVALID)~ THEN @11100570 /* ~<CHARNAME>, no debemos atacarle. Es una criatura con la inteligencia suficiente para razonar. Creo que deberíamos escucharle un poco más.~ */
== X3VIEJ IF ~InParty("X3Vie") InMyArea("X3Vie") !StateCheck("X3Vie",CD_STATE_NOTVALID)~ THEN @11100571 /* ~Ugh, ¿en serio entra en discusión dejar vivo a este monstruo? Sólo porque hable no hace que deje de ser un monstruo del que podemos sacar provecho de él. Es lo que el elfo anciano dijo. Yo digo que le matemos.~ */

== WILSONJ IF ~InParty("Wilson") InMyArea("Wilson") !StateCheck("Wilson",CD_STATE_NOTVALID)~ THEN @97050 /* ~(Bufido, lamento).~ */
== P_VORAG IF ~InParty("Wilson") InMyArea("Wilson")~ THEN @97051 /* No, buen oso. No estoy interesado en convertirte en piedra. Entiendo que te guste tu pelaje tal cual está. */
