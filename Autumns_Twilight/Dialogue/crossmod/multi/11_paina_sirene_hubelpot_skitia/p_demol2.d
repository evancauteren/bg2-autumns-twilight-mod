...
// Sirene crossmod
== C0SIRE2J IF ~InParty("C0Sirene") InMyArea("C0Sirene") !StateCheck("C0Sirene",CD_STATE_NOTVALID)~ THEN @10000046 /* ~Pequeño Demoledor, admiro tu valentía. ¡No irás sólo a esta batalla! Mientras pueda mantenerme en pie, ¡seré tu escudo!~ */

// Hubelpot crossmod
== SC#HubJ IF ~InParty("Hubelpot") InMyArea("Hubelpot") !StateCheck("Hubelpot",CD_STATE_NOTVALID)~ THEN @10000190 /*~Todo este asunto de la magia no la comprendo ni la comparto. ¡Muy bien, mi peludo amigo Demoledor! Cuentas con la asistencia de la buena de Chauntea a través de este humilde servidor. No veo las horas de celebrar con una buena sopa de maíz.~ */

// SKITIA NPCS
== X3EMIJ IF ~InParty("X3Emi") InMyArea("X3Emi") !StateCheck("X3Emi",CD_STATE_NOTVALID)~ THEN @11100530 /* ~Dusk, haré todo lo posible por dar lo mejor de mí en este combate. ¡Los nigromantes no deben vencer!~ */
== X3HELJ IF ~InParty("X3Hel") InMyArea("X3Hel") !StateCheck("X3Hel",CD_STATE_NOTVALID)~ THEN @11100531 /* ~De acuerdo, Asesino de Magos. ¡Es la hora de la guerra! No te contengas y desata la furia en la gloria del combate por venir.~ */
== X3KALJ IF ~InParty("X3Kal") InMyArea("X3Kal") !StateCheck("X3Kal",CD_STATE_NOTVALID)~ THEN @11100532 /* ~Entonces... ¿este es el final de tu viaje vengativo, Dusk? Si salimos en una pieza de esto, recuerda que es válido festejar en una taberna y que la ronda de cervezas irá a tu cuenta, ¿sí?~ */
== X3REBJ IF ~InParty("X3Reb") InMyArea("X3Reb") !StateCheck("X3Reb",CD_STATE_NOTVALID)~ THEN @11100533 /* ~El capítulo final de tu lucha contra la Orden de los Magos está a nuestro alcance, Dusk. Es un honor ser parte de esto. Ahora, ¡a luchar!~ */
== X3VIEJ IF ~InParty("X3Vie") InMyArea("X3Vie") !StateCheck("X3Vie",CD_STATE_NOTVALID)~ THEN @11100534 /* ~Hrmph. ¿Se supone que este grupo de hombrecillos son la epítome de la magia? Os demostraré lo equivocado que están...~ */
