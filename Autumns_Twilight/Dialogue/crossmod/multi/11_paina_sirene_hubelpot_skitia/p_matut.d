...
// Hubelpot Crossmod
== p_michel IF ~InParty("Hubelpot") InMyArea("Hubelpot") !StateCheck("Hubelpot",CD_STATE_NOTVALID)~ THEN @10000202 /* ~(Michelson huele con su nariz, corre hacia Hubelpot y luego vuelve hacia el cargamento. Parece que por su expresión no le gustó demasiado el olor a cebollas del druida. Al cabo de unos segundos de exageradas expresiones de desagrado, Michelson prácticamente se introduce en el cargamento y al salir muestra unas polvorientas bayas. Se acerca riendo hacia Hubelpot y se las entrega)~ */ DO ~ CreateItem ("p_bfitm2",1,1,0)  GiveItem("p_bfitm2", "Hubelpot") ~
== SC#HubJ IF ~InParty("Hubelpot") InMyArea("Hubelpot") !StateCheck("Hubelpot",CD_STATE_NOTVALID)~ THEN @10000203 /* ~¿Qué es esto? Parecen bayas solik. Tienen un olor dulzón. Bueno, supongo que me las habrás dado para que me las coma, ¿no es así, pequeño granuja? Tampoco me gustó tu expresión de desagrado ante mis ropas. Cocinar tiene sus contraproducencias, ¿sabes?~ */

// Paina Crossmod
== p_michel IF ~InParty("C0Paina") InMyArea("C0Paina") !StateCheck("C0Paina",CD_STATE_NOTVALID)~ THEN @9079218 /* ~(Michelson observa unos segundos a Pai'Na. Luego, con cierta dificultad, el leprechaun extrae del cargamento cormyreano una botella que parece más pesada de lo que aparenta. Con parsimonia, se acerca a la semi-drow y se la entrega cuidadosamente)~ */ DO ~ CreateItem ("p_nbk13",1,1,0)  GiveItem("p_nbk13", "C0Paina") ~
== C0PAINAJ IF ~InParty("C0Paina") InMyArea("C0Paina") !StateCheck("C0Paina",CD_STATE_NOTVALID)~ THEN @9079219 /* ~*olfatea* ¡¿Qué?! ¡¿Extracto de Escolopendra?! ¿Cómo has hecho para obtener ésto, criatura extraña? Hmpf. No importa. Le daré un buen uso.~ */


// Sirene Crossmod
== p_michel IF ~InParty("C0Sirene") InMyArea("C0Sirene") !StateCheck("C0Sirene",CD_STATE_NOTVALID)~ THEN @10000122 /* ~(Michelson saca un manto un poco polvoriento del cargamento. Lo sacude, quitándole por completo el polvillo y el mismo parece brillar de manera casi sutil. Se acerca a Sirene y se lo entrega solemnemente)~ */ DO ~ CreateItem ("p_nbk14",1,1,0)  GiveItem("p_nbk14", "C0Sirene") ~
== C0SIRE2J IF ~InParty("C0Sirene") InMyArea("C0Sirene") !StateCheck("C0Sirene",CD_STATE_NOTVALID)~ THEN @10000123 /* ~Gracias, buen Michelson. Es hermosa. Siento... siento su poder divino en su peso. Le daré un buen uso, os lo aseguro.~ */

// SKITIA
// EMILY
== p_michel IF ~InParty("X3Emi") InMyArea("X3Emi") !StateCheck("X3Emi",CD_STATE_NOTVALID)~ THEN @11100572 /* (Del cargamento, Michelson extrae un libro polvoriento, el cual sacude y, saltando, se acerca a Emily a entregárselo con solemnidad) */ DO ~ CreateItem ("p_cm01it",1,1,0)  GiveItem("p_cm01it", "X3Emi") ~
== X3EMIJ IF ~InParty("X3Emi") InMyArea("X3Emi") !StateCheck("X3Emi",CD_STATE_NOTVALID)~ THEN @11100573 /* ~¿Y esto? El libro, por alguna razón, me resulta conocido. Gracias, Michelson.~ */

// HELGA
== p_michel IF ~InParty("X3Hel") InMyArea("X3Hel") !StateCheck("X3Hel",CD_STATE_NOTVALID)~ THEN @11100574 /* (Michelson saca una botella que contiene un líquido, a primera vista, denso. Se acerca cauteloso a entregárselo a Helga) */ DO ~ CreateItem ("p_cm02it",1,1,0)  GiveItem("p_cm02it", "X3Hel") ~
== X3HELJ IF ~InParty("X3Hel") InMyArea("X3Hel") !StateCheck("X3Hel",CD_STATE_NOTVALID)~ THEN @11100575 /* ~Hmm. No confío en ti, pequeñuelo. ¿Cómo sé que esto no es una travesura de tu parte? De todas formas, veo que tiene un símbolo de Haela. Por ahora, lo tomaré. Pero, ¡ya verás si se trata de un engaño!~ */

// KALE
== p_michel IF ~InParty("X3Kal") InMyArea("X3Kal") !StateCheck("X3Kal",CD_STATE_NOTVALID)~ THEN @11100576 /* (Una pócima brillante aparece en las manos de Michelson y rápidamente va hacia Kale a entregársela con una sonrisa de oreja a oreja en su rostro) */ DO ~ CreateItem ("p_cm03it",1,1,0)  GiveItem("p_cm03it", "X3Kal") ~
== X3KALJ IF ~InParty("X3Kal") InMyArea("X3Kal") !StateCheck("X3Kal",CD_STATE_NOTVALID)~ THEN @11100577 /* ~Sí, sí. No sé por qué te emocionas tanto, duende. Oye, espera, tiene un aroma muy decente. Gracias, compañero. ~ */

// RECORDER
== p_michel IF ~InParty("X3Reb") InMyArea("X3Reb") !StateCheck("X3Reb",CD_STATE_NOTVALID)~ THEN @11100578 /* (Del cargamento, Michelson extrae lo que parece ser una arpa. Con rapidez, se la entrega a Recorder.) */ DO ~ CreateItem ("p_cm04it",1,1,0)  GiveItem("p_cm04it", "X3Reb") ~
== X3REBJ IF ~InParty("X3Reb") InMyArea("X3Reb") !StateCheck("X3Reb",CD_STATE_NOTVALID)~ THEN @11100579 /* ~Vaya, está fría al tacto. Espero poder sacar algunas notas propicias de esto, Michelson. Gracias.~ */

// VIENXAY 
== p_michel IF ~InParty("X3Vie") InMyArea("X3Vie") !StateCheck("X3Vie",CD_STATE_NOTVALID)~ THEN @11100580 /* (Un brillo surge del cargamento y Michelson extrae lo que parece ser una daga. Con presteza se dirige hacia Vienxay para dársela.) */ DO ~ CreateItem ("p_cm05it",1,1,0)  GiveItem("p_cm05it", "X3Vie") ~
== X3VIEJ IF ~InParty("X3Vie") InMyArea("X3Vie") !StateCheck("X3Vie",CD_STATE_NOTVALID)~ THEN @11100581 /* ~Ugh, ¿no tenías algo más bello para mí, renacuajo? Hm, aunque noto el poder de esta daga. Supongo que probaré usarla durante un tiempo a ver si me convence. Ahora, largo de aquí. ~ */
