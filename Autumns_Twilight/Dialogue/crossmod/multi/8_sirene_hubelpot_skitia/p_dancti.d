...
// SKITIA
// EMILY
== p_dancti IF ~InParty("X3Emi") InMyArea("X3Emi") !StateCheck("X3Emi",CD_STATE_NOTVALID)~ THEN @11100535 /* Aasimar, no temas a mi presencia. No tengo más que buenas intenciones para los aliados de Ardusk y <CHARNAME>. Tal es así, que este tomo fue diseñado especialmente para ti. Aprovechad el poder que os otorgará y serás una aliada aún más formidable para la empresa del Asesino de Magos. */ DO ~ CreateItem ("p_cm01it",1,1,0)  GiveItem("p_cm01it", "X3Emi") ~
== X3EMIJ IF ~InParty("X3Emi") InMyArea("X3Emi") !StateCheck("X3Emi",CD_STATE_NOTVALID)~ THEN @11100536 /* ~Y-yo... no sé si deba. Pero ya que <CHARNAME> y Dusk han dicho que eres de confianza, lo tomaré. Pero sólo lo leeré si <CHARNAME> me indica hacerlo.~ */

// HELGA
== p_dancti IF ~InParty("X3Hel") InMyArea("X3Hel") !StateCheck("X3Hel",CD_STATE_NOTVALID)~ THEN @11100537 /* Sirviente de Haela, guerrera formidable, esta pócima te hará más resistente en los arduos combates de tu porvenir. Que tu fuerza sea la guía de los mortales jóvenes que os rodean. */ DO ~ CreateItem ("p_cm02it",1,1,0)  GiveItem("p_cm02it", "X3Hel") ~
== X3HELJ IF ~InParty("X3Hel") InMyArea("X3Hel") !StateCheck("X3Hel",CD_STATE_NOTVALID)~ THEN @11100538 /* ~Hm. No me fío de ti, monstruo. Pero pese a que esto viene de ti, soy capaz de notar el influjo de Haela en esto. Lo aceptaré y ya veré si hago uso de esto o no. Ahora, aléjate de mí.~ */

// KALE
== p_dancti IF ~InParty("X3Kal") InMyArea("X3Kal") !StateCheck("X3Kal",CD_STATE_NOTVALID)~ THEN @11100539 /* Joven mediano, esto es para ti. Notarás que sus efectos mejorarán tus capacidades de combate. El sabor, también, debería ser de tu agrado. */ DO ~ CreateItem ("p_cm03it",1,1,0)  GiveItem("p_cm03it", "X3Kal") ~
== X3KALJ IF ~InParty("X3Kal") InMyArea("X3Kal") !StateCheck("X3Kal",CD_STATE_NOTVALID)~ THEN @11100540 /* ~*lo huele* Bueno... admito que su aroma es intoxicante. Ciertamente sabes cómo preparar tus pócimas, compañero. ~ */

// RECORDER
== p_dancti IF ~InParty("X3Reb") InMyArea("X3Reb") !StateCheck("X3Reb",CD_STATE_NOTVALID)~ THEN @11100541 /* Niña, esto es para ti. La música que saques de ella significará una gélida despedida para tus enemigos. */ DO ~ CreateItem ("p_cm04it",1,1,0)  GiveItem("p_cm04it", "X3Reb") ~
== X3REBJ IF ~InParty("X3Reb") InMyArea("X3Reb") !StateCheck("X3Reb",CD_STATE_NOTVALID)~ THEN @11100542 /* ~No me gusta cómo lo dices, pero debo admitir que esta arpa es hermosa. G-gracias, supongo. ~ */

// VIENXAY
== p_dancti IF ~InParty("X3Vie") InMyArea("X3Vie") !StateCheck("X3Vie",CD_STATE_NOTVALID)~ THEN @11100543 /* Ah, Vienxay, la Hechicera de las Sombras. Esta daga es para ti. Que no es engañe la simpleza de su diseño, pues hará que tu poder sea mayor cuando la empuñes. */ DO ~ CreateItem ("p_cm05it",1,1,0)  GiveItem("p_cm05it", "X3Vie") ~
== X3VIEJ IF ~InParty("X3Vie") InMyArea("X3Vie") !StateCheck("X3Vie",CD_STATE_NOTVALID)~ THEN @11100544 /* ~Ugh, es verdad. No es hermosa, pero desde ya, puedo sentir el poder en su mango. Has hecho bien en darme esto, demonio. Mis enemigos no tendrán ninguna oportunidad. ~ */
