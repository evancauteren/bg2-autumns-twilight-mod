...
// Hubelpot crossmod
== SC#HubJ IF ~InParty("Hubelpot") InMyArea("Hubelpot") !StateCheck("Hubelpot",CD_STATE_NOTVALID)~ THEN @10000194 /* ~¿Este es el clima que espera a Athkatla si dejamos que este gigantón gane este combate? No es bueno para las cosechas, <CHARNAME> *menea la cabeza* Nope, nope. Es mejor poner todo nuestro empeño para deshacernos de eso.~ */

// SKITIA NPCS
== X3EMIJ IF ~InParty("X3Emi") InMyArea("X3Emi") !StateCheck("X3Emi",CD_STATE_NOTVALID)~ THEN @11100557 /* ~Una buenas flechas de fuego te harán reflexionar, monstruo. ¡A la carga! ~ */
== X3HELJ IF ~InParty("X3Hel") InMyArea("X3Hel") !StateCheck("X3Hel",CD_STATE_NOTVALID)~ THEN @11100558 /* ~¿Crees que tienes la ventaja en esto, gólem? Los enanos estamos hechos para los combates bajo tierra. Venga, os lo demostraré. ~ */
== X3KALJ IF ~InParty("X3Kal") InMyArea("X3Kal") !StateCheck("X3Kal",CD_STATE_NOTVALID)~ THEN @11100559 /* ~¿Un templo bajo tierra para un hombre gigante de nieve? Espero os des cuenta el poco sentido que esto tiene, compañero. ~ */
== X3REBJ IF ~InParty("X3Reb") InMyArea("X3Reb") !StateCheck("X3Reb",CD_STATE_NOTVALID)~ THEN @11100560 /* ~He leído sobre Frostfell, <CHARNAME>. Todos los ataques de estos monstruos harán daño gélido. Tenlo en cuenta. ~ */
== X3VIEJ IF ~InParty("X3Vie") InMyArea("X3Vie") !StateCheck("X3Vie",CD_STATE_NOTVALID)~ THEN @11100561 /* ~Nada que una buena bola de fuego no pueda resolver, <CHARNAME>. Cubridme mientras extermino a estos alimañas con mi poder.~ */
